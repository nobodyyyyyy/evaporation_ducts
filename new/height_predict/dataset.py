    ...
    def _split(self, input_data, ratio_train, ratio_val, ratio_test=0.0, input_window=12, output_window=3,
        ...
        if ratio_test == 0.0:
            ...
            self.x_train, self.y_train, self.x_val, self.y_val = \
                torch.tensor(self.x_train).to(torch.float32), \
                torch.tensor(self.y_train).to(torch.float32), \
                torch.tensor(self.x_val).to(torch.float32), \
                torch.tensor(self.y_val).to(torch.float32)
            ...
            if machine_learning:
                self.x_train, self.y_train, self.x_val, self.y_val = \
                    self.x_train.numpy(), self.y_train.numpy(), \
                    self.x_val.numpy(), self.y_val.numpy()
