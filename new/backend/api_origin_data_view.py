...
def fetch_daily():
    ...
    date = data['date']
    _id = data['id']
    file = modules.dataUtil.get_origin_file_address(_id, date, prefix=_prefix)
    try:
...
def fetch_range():
    ...
    level = data['level']
    types = data['type']
    files, dates = modules.dataUtil.get_origin_file_addresses(_id, date_from, date_to, prefix=_prefix)
