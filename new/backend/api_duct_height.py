...
header_values = ['日期', '气温(°C)', '相对湿度(%)', '海温(°C)', '风速(m/s)', '压强(hPa)', '测量高度(m)',
                 'nps(m)', 'byc(m)', 'mgb(m)', 'pj(m)', '伪折射率模型(m)']
header_keys = ['date', 'temp', 'relh', 'sst', 'speed', 'pressure', 'height', 'nps', 'byc', 'mgb', 'pj', 'wei']

excel_header = ['日期', '气温', '相对湿度', '海温', '风速', '压强', '测量高度',
                'nps', 'babin', 'liuli2.0', 'pj', 'wei']
col_map = []
...
if not DEBUG:
    _excel_dir = '../../data/station_info.xlsx'
else:
    _excel_dir = './data/station_info.xlsx'
...
def init_api(excel_dir=_excel_dir):
    # 讀 excel 初始化信息
...
def get_graph_duct_height_data():
    ...
    dataset = pd.read_excel(file_addr)
    ret = [[],[],[],[],[]]
    header = ['nps', 'babin', 'liuli2.0', 'pj', 'wei']
    start = False
    dates = []
    for _, row in dataset.iterrows():
        cur_time = TimeUtil.str_to_timestamp(row['日期'])
